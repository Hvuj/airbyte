...
from source_bing_ads_by_id.cache import VcrCache
from source_bing_ads_by_id.client import Client
from source_bing_ads_by_id.reports import (
...
class BingAdsStream(Stream, ABC):
    ...
    def read_records(
        ...
        **kwargs: Mapping[str, Any],
        ...
        account_id = str(self.config['account_id']) if stream_slice else None
        customer_id = str(self.config['customer_id']) if stream_slice else None
    ...
    def stream_slices(
        ...
        **kwargs: Mapping[str, Any],
        ...
        for account in Accounts(self.client, self.config).read_records(SyncMode.full_refresh):
            ...
            # yield {"account_id": self.config["account_id"], "customer_id": self.config["customer_id"]}
        ...
        **kwargs: Mapping[str, Any],
        ...
        if self.config['account_id']:
            for campaign in campaigns.read_records(
                sync_mode=SyncMode.full_refresh, stream_slice={"account_id": self.config["account_id"], "customer_id": self.config["customer_id"]}
            ):
                yield {"campaign_id": campaign["Id"], "account_id": self.config["account_id"], "customer_id": self.config["customer_id"]}
        ...
        **kwargs: Mapping[str, Any],
        ...
        if self.config['account_id']:
            for ad_group in ad_groups.read_records(sync_mode=SyncMode.full_refresh, stream_slice=slice):
                yield {"ad_group_id": ad_group["Id"], "account_id": self.config["account_id"], "customer_id": self.config["customer_id"]}
        yield from []
...
class SourceBingAdsById(AbstractSource):
    """
    ...
    def check_connection(self, logger: AirbyteLogger, config: Mapping[str, Any]) -> Tuple[bool, Any]:
        try:
            ...
            account_ids = {str(config["account_id"])}
            if account_ids:
                return True, None
            else:
